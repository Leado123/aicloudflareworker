...
async function testSpanishConjugationAPI() {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.questions.length > 0) {
        ...
        console.log(
          `Sample question: ${q.verbInInfiniteTense} → ${q.conjugatedVerbAnswer}`
        );
    ...
    if (data.success) {
      ...
      data.questions.forEach((q, index) => {
        ...
        if (q.exampleSentenceWithDifferentPronoun) {
          console.log(
            `   Alternative: ${q.exampleSentenceWithDifferentPronoun}`
          );
        }
      });
    } else {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log(
        `Generated ${data.questions.length} questions despite invalid params`
      );
    } else {
