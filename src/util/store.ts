...
export function createNewCraftingBench(title: string = "New Crafting Bench"): string {
    const id = crypto.randomUUID();
...
export function updateCraftingBench(id: string, updates: Partial<CraftingBench>) {
    const currentBenches = craftingBenches.get();
    craftingBenches.set(
...
export function removeFileFromCraftingBench(benchId: string, fileIndex: number) {
    const bench = craftingBenches.get().find(b => b.id === benchId);
    if (bench) {
...
export function saveFlashcardsToCraftingBench(benchId: string, flashcards: FlashCard[]) {
    updateCraftingBench(benchId, { 
        flashcards,
...
export function addFlashcardToCraftingBench(benchId: string, flashcard: Omit<FlashCard, 'id'>) {
    const bench = craftingBenches.get().find(b => b.id === benchId);
    if (bench) {
...
export function removeFlashcardFromCraftingBench(benchId: string, flashcardId: string) {
    const bench = craftingBenches.get().find(b => b.id === benchId);
    if (bench) {
        const updatedFlashcards = bench.flashcards.filter(card => card.id !== flashcardId);
        saveFlashcardsToCraftingBench(benchId, updatedFlashcards);
    }
}

export function updateFlashcard(benchId: string, flashcardId: string, updates: Partial<FlashCard>) {
    const bench = craftingBenches.get().find(b => b.id === benchId);
    if (bench) {
...
export async function addFilesToCraftingBenchWithData(benchId: string, newFiles: File[]) {
    const bench = craftingBenches.get().find(b => b.id === benchId);
    if (bench) {
