...
export default function StoreManager({ children }: StoreManagerProps) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      try {
        ...
        const savedCurrentBenchId = localStorage.getItem(
          "currentCraftingBenchId"
        );

        let conversationsArray: any[] = [];
        if (savedConversations && savedConversations !== "null") {
          conversationsArray = JSON.parse(savedConversations).map((conv) => ({
            ...conv,
            messages: Array.isArray(conv.messages)
              ? conv.messages.map((msg) => {
                  // Try to parse assistant message content if it's a stringified object
                  if (
                    msg.role === "assistant" &&
                    ...
                    try {
                      ...
                      // If it has a response field, treat as new schema
                      if (
                        parsed &&
        ...
        if (
          ...
          const conversationExists = conversationsArray.some(
            (conv) => conv.id === savedCurrentId
          );
        ...
        if (savedCraftingBenches && savedCraftingBenches !== "null") {
          ...
          benchesArray = benchesArray.map((bench) => ({
            ...bench,
            files: (bench.storedFiles || []).map((storedFile: any) =>
              storedDataToFile(storedFile)
            ),
            storedFiles: bench.storedFiles || [],
            notes: bench.notes || "",
            flashcards: bench.flashcards || [],
            lastNotesUpdate: bench.lastNotesUpdate
              ? new Date(bench.lastNotesUpdate)
              : undefined,
            lastFlashcardsUpdate: bench.lastFlashcardsUpdate
              ? new Date(bench.lastFlashcardsUpdate)
              : undefined,
          }));

          // Debug logging for loaded content
          const benchesWithContent = benchesArray.filter(
            (b) => b.notes || b.flashcards.length > 0
          );
          if (benchesWithContent.length > 0) {
            console.log(
              "Loaded crafting benches with content:",
              benchesWithContent.map((b) => ({
                id: b.id,
                title: b.title,
                hasNotes: !!b.notes,
        ...
        if (
          savedCurrentBenchId &&
          savedCurrentBenchId !== "null" &&
          savedCurrentBenchId !== ""
        ) {
          // Check if the crafting bench still exists
          const benchExists = benchesArray.some(
            (bench) => bench.id === savedCurrentBenchId
          );
          if (benchExists) {
        ...
        if (
          savedContentMode &&
          Object.values(CONTENT_MODE).includes(savedContentMode as any)
        ) {
          contentMode.set(
            savedContentMode as (typeof CONTENT_MODE)[keyof typeof CONTENT_MODE]
          );
        } else {
  ...
  useEffect(() => {
    ...
    const unsubscribeConversations = conversations.subscribe((convs) => {
      if (typeof window !== "undefined") {
        try {
          // Serialize assistant messages with object content as JSON strings
          const serializableConvs = convs.map((conv) => ({
            ...conv,
            messages: Array.isArray(conv.messages)
              ? conv.messages.map((msg) => {
                  if (
          ...
          localStorage.setItem(
            "conversations",
            JSON.stringify(serializableConvs)
          );
        } catch (error) {
    ...
    const unsubscribeCurrentId = currentConversationId.subscribe((id) => {
      if (typeof window !== "undefined") {
        try {
          ...
          console.warn(
            "Failed to save current conversation ID to localStorage:",
            error
          );
        }
      }
    ...
    const unsubscribeCraftingBenches = craftingBenches.subscribe((benches) => {
      if (typeof window !== "undefined") {
        try {
          ...
          localStorage.setItem(
            "craftingBenches",
            JSON.stringify(serializedBenches)
          );

          // Debug logging for notes and flashcards
          const benchesWithContent = serializedBenches.filter(
            (b) => b.notes || b.flashcards.length > 0
          );
          if (benchesWithContent.length > 0) {
            console.log(
              "Saved crafting benches with content:",
              benchesWithContent.map((b) => ({
                id: b.id,
                title: b.title,
                hasNotes: !!b.notes,
          ...
          console.warn(
            "Failed to save crafting benches to localStorage:",
            error
          );
        }
      }
    ...
    const unsubscribeCurrentBenchId = currentCraftingBenchId.subscribe((id) => {
      if (typeof window !== "undefined") {
        try {
          ...
          console.warn(
            "Failed to save current crafting bench ID to localStorage:",
            error
          );
        }
      }
